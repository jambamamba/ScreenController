...
protected:
    void StartEncoderThread(int width = 0,
                            ...
                            std::function<void(EncodedChunk enc)> encoderOutputFn = nullptr
                    );
    ...
    std::function<void(EncodedChunk enc)> _encoderOutputFn = nullptr;
...
struct X265Decoder
{
    ...
    virtual ~X265Decoder();
    void Decode(uint32_t ip, ssize_t width, ssize_t height, const EncodedChunk &enc);
protected:
    std::future<void> StartDecoderThread();
    HevcReader _hevc;
    YuvWriter _yuv;
    std::future<void> _decoder_thread;
    EncodedChunk _img_to_decode;
    std::atomic<bool> _die = false;
};
